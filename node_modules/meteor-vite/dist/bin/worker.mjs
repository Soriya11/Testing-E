...
// src/meteor/IPC/interface.ts
function CreateIPCInterface(methods) {
  return methods;
}
function validateIpcChannel(send) {
  if (typeof process.send !== "function") {
    throw new Error("Worker was not launched with an IPC channel!");
  }
}

// src/meteor/IPC/methods/build.ts
...
var MeteorStubs = setupPlugin(async () => {
  return {
    ...
    async setupContext(viteId, server2, pluginSettings) {
      return ViteLoadRequest.prepareContext({ id: viteId, pluginSettings, server: server2 });
    },
    async load(request) {
  ...
  const createPlugin = async () => {
    ...
    return {
      ...
      async load(viteId) {
        ...
        const request = await plugin.setupContext(viteId, server2, settings);
...
function mergeMeteorSettings(userConfig, defaults, overrides) {
  const viteConfig2 = parseConfig(userConfig);
  const existingSettings = viteConfig2.meteor || {};
  const withDefaults = mergeWithTypes(defaults, existingSettings);
  return viteConfig2.meteor = mergeWithTypes(withDefaults, overrides);
}
function mergeViteSettings(userConfig, defaults) {
  const viteConfig2 = parseConfig(userConfig);
  return mergeWithTypes(defaults, viteConfig2);
}

// src/meteor/IPC/methods/build.ts
var build_default = CreateIPCInterface({
  async "vite.build"(reply, buildConfig) {
    try {
      const { viteConfig: viteConfig2, inlineBuildConfig, outDir } = await prepareConfig(buildConfig);
      ...
      reply({
        ...
        data: {
          payload: {
            outDir,
            success: true,
            meteorViteConfig: viteConfig2.meteor,
  ...
  async "tsup.watch.meteor-vite"(reply) {
    const npmPackagePath = Path5.join(process.cwd(), "/node_modules/meteor-vite/");
    const tsupPath = Path5.join(npmPackagePath, "/node_modules/.bin/tsup-node");
...
async function prepareConfig(buildConfig) {
  const { meteor, packageJson } = buildConfig;
  const configFile = buildConfig.packageJson?.meteor?.vite?.configFile ?? buildConfig.packageJson?.meteor?.viteConfig;
  Object.entries(buildConfig).forEach(([key, value]) => {
    if (!value) {
      throw new Error(`Vite: Worker missing required build argument "${key}"!`);
    }
  });
  const viteConfig2 = await resolveConfig({
    configFile,
    plugins: [
      meteorWorker({})
  ...
  if (!viteConfig2.meteor?.clientEntry) {
    throw new Error(`You need to specify an entrypoint in your Vite config! See: ${package_default.homepage}`);
  }
  const outDir = Path5.join(viteConfig2.meteor.tempDir, "bundle");
  return {
    ...
    inlineBuildConfig: {
      ...
      build: {
        lib: {
          entry: viteConfig2.meteor.clientEntry,
          formats: ["es"]
        },
        rollupOptions: {
          output: {
            entryFileNames: "meteor-entry.js",
            chunkFileNames: viteConfig2.meteor.chunkFileNames ?? "[name]-[hash:12].js"
          }
        },
        outDir,
        minify: false
...
function validateOutput(rollupResult) {
  if (!rollupResult) {
...
// src/meteor/IPC/methods/vite-server.ts
import FS5 from "fs/promises";
import Path6 from "path";
import { createServer, resolveConfig as resolveConfig2 } from "vite";
var server;
var viteConfig;
var listening = false;
var vite_server_default = CreateIPCInterface({
  async "vite.server.getConfig"(replyInterface) {
    await sendViteConfig(replyInterface);
  },
  async "meteor.events.emit"(reply, data) {
    MeteorEvents_default.ingest(data);
  },
  // todo: Add reply for triggering a server restart
  async "vite.server.start"(replyInterface, { packageJson, meteorParentPid }) {
    ...
    if (backgroundWorker.isRunning) {
      ...
      Logger_default.info(`Vite server running as background process. (pid ${backgroundWorker.config.pid})`);
      return process.exit(0);
    }
    const server2 = await createViteServer({
      packageJson,
      refreshNeeded: () => {
        replyInterface({
          kind: "refreshNeeded",
  ...
  async "vite.server.stop"() {
    if (!server)
      return;
    try {
      Logger_default.info("Shutting down vite server...");
      await server.close();
      Logger_default.info("Vite server shut down successfully!");
    } catch (error) {
      Logger_default.error("Failed to shut down Vite server:", error);
    }
  }
});
async function createViteServer({
  ...
  viteConfig = await resolveConfig2({
    configFile: packageJson?.meteor?.vite?.configFile ?? packageJson?.meteor?.viteConfig
  }, "serve");
    ...
    plugins: [
      ...
      },
      {
        ...
        configureServer: (server2) => {
          server2.middlewares.use("/__meteor__/ipc-message", (req, res, next) => {
            let body = "";
            req.on("data", (chunk) => {
              body += chunk.toString();
            });
            req.on("end", () => {
              ...
              MeteorEvents_default.ingest(message);
              res.statusCode = 204;
              next();
            });
          });
...
async function sendViteConfig(reply) {
  if (!server) {
    Logger_default.debug("Tried to get config from Vite server before it has been created!");
    return;
  }
  const { config } = server;
...
var BackgroundWorker = class _BackgroundWorker {
  constructor(config) {
    this.config = config;
  }
  static instance;
  static configPath = process.env.BACKGROUND_WORKER_PID_PATH || Path6.join(".meteor", "local", "vite", "vite-dev-server.pid");
  static async init(meteorParentPid) {
    if (_BackgroundWorker.instance) {
      return _BackgroundWorker.instance;
    }
    const myConfig = {
      pid: process.pid,
    ...
    try {
      await FS5.mkdir(Path6.dirname(this.configPath), { recursive: true });
      const content = await FS5.readFile(this.configPath, "utf-8");
      const config = JSON.parse(content);
      _BackgroundWorker.instance = new _BackgroundWorker(config);
    } catch (error) {
      _BackgroundWorker.instance = new _BackgroundWorker(myConfig);
    }
    const worker = _BackgroundWorker.instance;
    if (!worker.isRunning) {
      await worker.update(myConfig);
      worker._watchForParentExit();
    } else {
      Logger_default.debug(`Background worker should be running with PID: ${worker.config.pid}`, worker.config);
    }
    return worker;
  }
  _watchForParentExit() {
    setInterval(() => {
      ...
      Logger_default.warn("Meteor parent process is no longer running. Shutting down...");
      this.update({
        pid: 0,
        meteorPid: 0,
        meteorParentPid: 0,
  ...
  get isRunning() {
    if (!this.config.pid) {
      Logger_default.debug("No background worker process ID");
      return false;
    }
    if (this.config.pid === process.pid) {
      Logger_default.debug(`Background worker's process ID is identical to ours`);
      return false;
    }
    if (!this._isRunning(this.config.pid)) {
      Logger_default.debug(`Background worker not running: ${this.config.pid} (current PID ${process.pid}) `);
      return false;
    }
    return true;
  }
  async update(config) {
    this.config = config;
    await FS5.writeFile(_BackgroundWorker.configPath, JSON.stringify(this.config));
  }
  async setViteConfig(viteConfig2) {
    if (this.config.pid !== process.pid && this.isRunning) {
      Logger_default.debug(`Skipping Vite config write - config is controlled by different background process: ${this.config.pid}`);
      return;
    }
    await this.update({
      ...this.config,
...
// src/meteor/IPC/methods/index.ts
var IpcMethods = {
  ...vite_server_default,
  ...build_default
};
var methods_default = IpcMethods;
...
process.on("message", async (message) => {
  ...
  const callWorkerMethod = methods_default[message.method];
