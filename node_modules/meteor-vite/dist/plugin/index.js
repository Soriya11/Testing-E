...
var divColor = (text) => import_picocolors.default.dim(text);
var MeteorViteError = class extends Error {
  ...
  addSection(title, object) {
    const content = (0, import_util.inspect)(object, { colors: true });
  ...
  async beautify() {
    ...
    const moduleString = moduleId && import_picocolors.default.yellow(`
\u26A1   <${moduleId}>`) || "";
    this.name = `
...
${this.titleDivider({
    ...
    this.message = [
      "",
      "",
      `${import_picocolors.default.bgRed(import_picocolors.default.bold(" ERROR "))} ${this.message}`,
      `${import_picocolors.default.dim(this.subtitle)}`,
      "",
      ...this.metadataLines,
      this.titleDivider({
        title: "[Error Stack]",
...
var createLabelledLogger = (label) => createLogger((message, dataLines) => {
  ...
  const data = dataLines.map(([key, value]) => {
    return `
 ${import_picocolors2.default.dim("L")}  ${key}: ${value}`;
  }).join("");
...
var AutoImportQueue = class {
  restartTimeout;
  addedPackages = [];
  queueWrite = (0, import_p_limit.default)(1);
  onRestartWatchers = [];
  /**
  ...
  async write({ importString, meteorEntrypoint, skipRestart }) {
    ...
    const content = await import_promises.default.readFile(meteorEntrypoint, "utf-8");
    ...
    await this.queueWrite(async () => {
      const content2 = await import_promises.default.readFile(meteorEntrypoint, "utf-8");
      const newContent = viteAutoImportBlock({
        id: importString,
        content: content2
      });
      await import_promises.default.writeFile(meteorEntrypoint, newContent);
    ...
    if (this.addedPackages.length > lastPackageCount && !skipRestart) {
      await MeteorEvents_default.waitForMessage({
        ...
        if (error instanceof EventTimeout) {
          Logger_default.warn(`Timed out waiting for Meteor to refresh the client for ${import_picocolors3.default.yellow(importString)}!`);
...
var import_types3 = require("@babel/types");
...
var ParserError = class extends MeteorViteError {
  ...
  async formatLog() {
    ...
    if (node) {
      this.addLine([(0, import_util2.inspect)(node)]);
    }
  }
};
...
var PackageModule = class {
  ...
  getModuleExportsAssignment(node) {
    if (!(0, import_types.isAssignmentExpression)(node))
      return;
    if (!(0, import_types.isMemberExpression)(node.left))
      return;
    if (!(0, import_types.isIdentifier)(node.left.object, { name: "module" }))
      return;
    if (!(0, import_types.isIdentifier)(node.left.property, { name: "exports" }))
      return;
    return node.right;
  }
  getCjsExportAssignment(node) {
    ...
    if (!(0, import_types.isMemberExpression)(node.left))
      return;
    if (!(0, import_types.isIdentifier)(node.left.object, { name: "exports" }))
      return;
    if (!(0, import_types.isIdentifier)(node.left.property))
      return;
  ...
  parse(node) {
    ...
    if (moduleExports) {
      if ((0, import_types.isCallExpression)(moduleExports)) {
        if (!(0, import_types.isIdentifier)(moduleExports.callee, { name: "require" }))
          return;
        if (!(0, import_types.isStringLiteral)(moduleExports.arguments[0]))
          return;
      ...
      if (!(0, import_types.isObjectExpression)(moduleExports)) {
        return;
      }
...
var MeteorInstall = class {
  ...
  static parseNpmInstall(node) {
    ...
    for (const directory of node_modules.value.properties) {
      if (!(0, import_types2.isObjectExpression)(directory.value))
        return;
  ...
  static isMeteorInstall(expression) {
    ...
    if (expression.type !== "CallExpression")
      return false;
    if (!(0, import_types2.is)("Identifier", expression.callee, { name: "meteorInstall" }))
      return false;
    return true;
  }
  traverseModules(properties, parentPath) {
    properties.forEach((property) => {
      ...
      const module2 = new PackageModule(path);
      if (property.value.type === "ObjectExpression") {
        return this.traverseModules(property.value.properties, `${path}/`);
      }
      (0, import_types2.traverse)(property.value.body, {
        enter(node) {
          module2.parse(node);
        }
      });
      this.modules[path] = module2.exports;
...
async function parseMeteorPackage({ fileContent, filePath }) {
  ...
  const content = fileContent || import_promises2.default.readFile(filePath, "utf-8");
...
function parseSource(code) {
  return new Promise((resolve, reject) => {
    const source = (0, import_parser.parse)(code);
    const result = {
      name: "",
      modules: {},
      packageScopeExports: {},
    ...
    (0, import_types3.traverse)(source, {
      enter(node) {
...
function readMainModulePath(node) {
  if (node.type !== "VariableDeclarator")
    return;
  if (!(0, import_types3.is)("Identifier", node.id, { name: "exports" }))
    return;
  if (node.init?.type !== "CallExpression")
    return;
  if (!(0, import_types3.is)("Identifier", node.init.callee, { name: "require" }))
    return;
...
function parsePackageScope(node) {
  ...
  function meteorV3(node2) {
    if (!(0, import_types3.isCallExpression)(node2))
      return;
    if (!(0, import_types3.isMemberExpression)(node2.callee))
      return;
    if (!(0, import_types3.isIdentifier)(node2.callee.property, { name: "queue" }))
      return;
    if (!(0, import_types3.isStringLiteral)(node2.arguments[0]))
      return;
    const packageName = node2.arguments[0].value;
    let packageClosure = null;
    const exports2 = [];
    if ((0, import_types3.isFunctionExpression)(node2.arguments[1])) {
      packageClosure = node2.arguments[1];
    }
    if ((0, import_types3.isFunctionExpression)(node2.arguments[2])) {
    ...
    for (const node3 of packageClosure.body.body) {
      if (!(0, import_types3.isReturnStatement)(node3))
        continue;
      if (!(0, import_types3.isObjectExpression)(node3.argument))
        continue;
      for (const property of node3.argument.properties) {
        if (!(0, import_types3.isObjectProperty)(property))
          continue;
        if (!(0, import_types3.isIdentifier)(property.key, { name: "export" }))
          continue;
        if (!(0, import_types3.isFunctionExpression)(property.value))
          continue;
        const exportBody = property.value.body.body;
        for (const node4 of exportBody) {
          if (!(0, import_types3.isReturnStatement)(node4))
            continue;
          if (!(0, import_types3.isObjectExpression)(node4.argument))
            continue;
          node4.argument.properties.forEach((node5) => {
            if (!(0, import_types3.isObjectProperty)(node5))
              return;
            if (!(0, import_types3.isIdentifier)(node5.key))
              return;
...
var PackageExport = class {
  ...
  constructor({ meteorPackage, key, packageName }) {
    ...
    if (meteorPackage.name !== packageName) {
      Logger_default.warn(`Detected multiple package definitions within ${import_picocolors4.default.yellow(this.meteorPackage.name)}! Please report this to ${package_default.bugs.url}`);
      Logger_default.warn(`Package ${import_picocolors4.default.red(packageName)} was defined within ${import_picocolors4.default.yellow(meteorPackage.name)} \u{1F914}`);
    }
  }
  serialize() {
  ...
  validateNewKey(entry) {
    ...
    if (existing instanceof ModuleExport) {
      throw new ConflictingExportKeys(
        `Duplicate module export detected in ${import_picocolors5.default.yellow(entry.parentModule.meteorPackage.packageId)}!`,
        { conflict: { thisExport: entry, conflictedWith: existing } }
      );
    }
    if (existingReExport) {
      throw new ConflictingExportKeys(
        `Export key is conflicting with a module re-export in ${import_picocolors5.default.yellow(entry.parentModule.meteorPackage.packageId)}!`,
...
function isSameModulePath(options) {
  const fileA = import_path.default.parse(options.filepathA.replace(REGEX_LEADING_SLASH, ""));
  const fileB = import_path.default.parse(options.filepathB.replace(REGEX_LEADING_SLASH, ""));
...
var ViteLoadRequest = class _ViteLoadRequest {
  constructor(context) {
    ...
    this.log = createLabelledLogger(`[${import_picocolors6.default.yellow(context.id.replace("meteor/", ""))}]`);
  ...
  static loadFileData({ id, pluginSettings: { meteorStubs } }) {
    ...
    const sourcePath = import_path2.default.join(meteorStubs.meteor.packagePath, sourceFile);
    const resolverResultCache = JSON.parse(import_fs.default.readFileSync(import_path2.default.join(
    ...
    const manifestPath = {
      local: import_path2.default.join(meteorStubs.meteor.isopackPath, sourceName, "web.browser.json"),
      globalCache: import_path2.default.join(globalMeteorPackagesDir, sourceName, packageVersion, "web.browser.json")
    };
    const content = import_promises3.default.readFile(sourcePath, "utf-8").catch((error) => {
    ...
    return {
      ...
      importPath,
      sourcePath,
      manifestPath: import_fs.default.existsSync(manifestPath.local) ? manifestPath.local : manifestPath.globalCache
    };
  }
  /**
  ...
  static async loadManifest({ file }) {
    if (!(0, import_fs.existsSync)(file.manifestPath)) {
      return;
    }
    return JSON.parse(await import_promises3.default.readFile(file.manifestPath, "utf8"));
  }
  /**
  ...
  static guessMeteorPackagePath() {
    ...
    parts.forEach((part) => {
      ...
      packagePath = import_path2.default.posix.join(packagePath, part);
    });
    return import_path2.default.join(packagePath, "../");
  }
  /**
  ...
  async forceImport() {
    ...
    const meteorClientEntryFile = import_path2.default.resolve(process.cwd(), mainModule.client);
    if (!(0, import_fs.existsSync)(meteorClientEntryFile)) {
  ...
  return {
    ...
    async validateConfig({ meteorStubs }) {
      if (!meteorStubs.packageJson) {
        ...
        meteorStubs.packageJson = JSON.parse(await import_promises4.default.readFile(jsonPath, "utf-8"));
      }
...
async function storeDebugSnippet({ request, stubTemplate: stubTemplate2, meteorPackage }) {
  const baseDir = import_path3.default.join(request.context.pluginSettings.tempDir, "stubs", request.context.file.packageId.replace(":", "_"));
  const templatePath = import_path3.default.join(baseDir, request.context.file.importPath || "", "template.js");
  const packagePath = import_path3.default.join(baseDir, "package.js");
  const parserPath = import_path3.default.join(baseDir, "parsed.json");
  await import_promises4.default.mkdir(import_path3.default.dirname(templatePath), { recursive: true });
  await Promise.all([
    import_promises4.default.writeFile(templatePath, stubTemplate2),
    import_promises4.default.writeFile(packagePath, await request.context.file.content),
    import_promises4.default.writeFile(parserPath, meteorPackage.toJson())
  ]);
  request.log.info("Stored debug snippets", {
    File: import_picocolors7.default.cyan(baseDir)
  });
}
...
function mergeWithTypes(defaults, overrides) {
  return (0, import_vite.mergeConfig)(defaults, overrides);
}
...
function meteorWorker(config) {
  const METEOR_LOCAL_DIR = process.env.METEOR_LOCAL_DIR || import_path4.default.join(".meteor", "local");
  let enforce;
  let resolveId;
  if (config.externalizeNpmPackages) {
    enforce = "pre";
    resolveId = function resolveId2(id) {
      const [module2, ...path] = id.split("/");
      ...
      config: (userConfig) => {
        const pluginSettings = mergeMeteorSettings(userConfig, {
          meteorStubs: {
            ...
            meteor: {
              packagePath: import_path4.default.join(METEOR_LOCAL_DIR, "build", "programs", "web.browser", "packages"),
              isopackPath: import_path4.default.join(METEOR_LOCAL_DIR, "isopacks")
            }
          },
          tempDir: import_path4.default.join(METEOR_LOCAL_DIR, "vite"),
          stubValidation: {
      ...
      configResolved(resolvedConfig) {
        ...
        if (!config2.meteor) {
          throw new FatalMeteorViteError(
            `Could not retrieve Meteor-Vite settings! Did you forget to add ${import_picocolors8.default.yellow("meteor-vite")} to your Vite config?`,
            {
